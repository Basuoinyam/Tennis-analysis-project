...
class MiniCourt():
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,player_boxes,ball_boxes,original_court_key_points):
        ...
        for frame_num , player_bboxes in enumerate(player_boxes):
            ...
            closest_player_id_to_ball=min(player_bboxes.keys(),key= lambda x : measure_distance(ball_position,get_center_of_bbox(player_bboxes[x])))
            output_player_bbox_dict={}
            for player_id , box in player_bboxes.items():
                ...
                player_height_in_pixels=max(bboxes_height_in_pixels)
                mini_court_player_position=self.get_mini_court_coordinates(
                    foot_position,closest_keypoint,
        ...
        return output_ball_boxes,output_player_boxes


    def get_mini_court_coordinates(self,object_coordinates,
                                   closest_keypoints,
                                   closest_keypoint_indices,
                                   player_height_in_pixels,
                                   player_height_in_meters):

        distance_from_keypoints_x_pixels,distance_from_keypoints_y_pixels=measure_xy_distance(object_coordinates,closest_keypoints)
        ...
        distance_from_keypoints_y_meters=convert_pixel_distance_to_meters(distance_from_keypoints_y_pixels,
                                                                          player_height_in_meters,
                                                                          player_height_in_pixels)
        mini_court_x_pixels=self.convert_meters_to_pixels(distance_from_keypoints_x_meters)
        mini_court_y_pixels=self.convert_meters_to_pixels(distance_from_keypoints_y_meters)
        closest_mini_court_keypoints=(
            self.drawing_key_points[closest_keypoint_indices*2],
            self.drawing_key_points[closest_keypoint_indices*2+1],
        )
        mini_court_player_position=(
            closest_mini_court_keypoints[0]+mini_court_x_pixels,
            closest_mini_court_keypoints[1]+mini_court_y_pixels,
        )
        return mini_court_player_position
    def drow_point_in_mini_court(self,frames,positions,color=(0,255,0)):
